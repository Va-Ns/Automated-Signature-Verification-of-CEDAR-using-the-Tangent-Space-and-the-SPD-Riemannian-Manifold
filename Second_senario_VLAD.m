...
for Iter = 1 : numIters
    ...
    % Randomly sample the indices every time for every folder. In this way, we retrieve half of the 
    % dataset's writers in a randomly sampled manner, here 27 Writers
    ...
    % random indices created above by deleting the sampled ones. So here we would have the remaining
    % 28 Writers
    ...
    for Fold = 1 : numFolds
        ...
        % references, we need to perform Dichotomy Transform between every possible combination of 
        % those 17 references.
        ...
        %% Form the ω(-) class
         ...
         % Genuine sample of another Writer. Here we employ a pairing between a Genuine sample of a
         % Writer with the simulated-or-skilled sample of the same Writer.
        ...
        Data.G_Training_set = G_Training_set;
        Data.F_Training_set = F_Training_set;
        Data.G_Val_set = G_Val_set;
        Data.F_Val_set = F_Val_set;

        % Concentrate the Dictionaries into a struct

        Codebooks.G_Train_Codebook = G_Train_Codebook;
        Codebooks.F_Train_Codebook = F_Train_Codebook;
        Codebooks.G_Val_Codebook = G_Val_Codebook;
        Codebooks.F_Val_Codebook = F_Val_Codebook;

        VLAD_Encoding{Iter,Fold} = VLADNV(Codebooks,Data);

        fprintf('    Finished VLAD Encoding of Iter %d of Fold %d \n', Iter,Fold)

        %% Create the labels for the data
         
        % Create the labels for the Training set of both Genuine and Forgery
        G_Training_Label = ones([1 size(VLAD_Encoding{Iter,Fold}.G_Train_VLAD_Encoding_normed,1)])';
        F_Training_Label = -ones([1 size(VLAD_Encoding{Iter,Fold}.F_Train_VLAD_Encoding_normed,1)])';

        % Create the labels for the Validation set of both Genuine and Forgery
        G_Val_Label = ones([1 size(VLAD_Encoding{Iter,Fold}.G_Val_VLAD_Encoding_normed,1)])';
        F_Val_Label = -ones([1 size(VLAD_Encoding{Iter,Fold}.F_Val_VLAD_Encoding_normed,1)])';
        ...
        Training_Matrix = [VLAD_Encoding{Iter,Fold}.G_Train_VLAD_Encoding_normed;
                           VLAD_Encoding{Iter,Fold}.F_Train_VLAD_Encoding_normed];

        Validation_Matrix = [VLAD_Encoding{Iter,Fold}.G_Val_VLAD_Encoding_normed;
                             VLAD_Encoding{Iter,Fold}.F_Val_VLAD_Encoding_normed];
        ...
        Best_Mdl(Iter).Codebooks{Fold} = Codebooks;
